...
func TestCacheManager(t *testing.T) {
	...
	cm2, err := NewCacheManager("test2", "nonexistent_cache", time.Second*3, 10)
	if err != nil {
		t.Fatalf("failed to create cache manager: %v", err)
	}
	cm2.Set("key4", "value4")
	time.Sleep(time.Second * 4)
	cm3, err := NewCacheManager("test3", "nonexistent_cache", time.Second*3, 10)
	if err != nil {
		t.Fatalf("failed to create cache manager: %v", err)
	}
	all3 := cm3.GetAll()
...
func TestMetaInfoCacheManager(t *testing.T) {
	...
	cm, err := NewCacheManager("metaTest1", "test_meta_cache", time.Second*3, 10)
	if err != nil {
		t.Fatalf("failed to create cache manager: %v", err)
	}
	...
	// Test cache file creation and loading
	cm2, err := NewCacheManager("metaTest2", "nonexistent_meta_cache", time.Second*3, 10)
	if err != nil {
		t.Fatalf("failed to create cache manager: %v", err)
	}
	cm2.Set("key4", metadataInfo4)
	time.Sleep(time.Second * 4)
	cm3, err := NewCacheManager("test3", "nonexistent_meta_cache", time.Second*3, 10)
	if err != nil {
		t.Fatalf("failed to create cache manager: %v", err)
	}
	all3 := cm3.GetAll()
