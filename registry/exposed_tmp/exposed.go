...
func RegisterServiceInstanceContext(ctx context.Context) error {
	defer func() {
		...
		if err := recover(); err != nil {
			logger.Errorf("[Registry] register service instance failed, please check if registry protocol is imported, err=%v", err)
		}
	}()
...
func UnregisterServiceInstance() error {
	defer func() {
		if err := recover(); err != nil {
			logger.Errorf("[Registry] unregister service instance failed, please check if registry protocol is imported, err=%v", err)
		}
	}()
	protocol := extension.GetProtocol(constant.RegistryKey)
	if rf, ok := protocol.(registry.RegistryFactory); ok {
