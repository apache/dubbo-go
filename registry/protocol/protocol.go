...
func (proto *registryProtocol) getRegistry(registryUrl *common.URL) registry.Registry {
	...
	actualReg, _ := proto.registries.LoadOrStore(cacheKey, func() any {
		...
		if err != nil {
			logger.Errorf("[Registry][Protocol] registry cannot connect successfully, err=%s", err.Error())
			panic(err)
		}
		return reg
	}())
...
func (proto *registryProtocol) Refer(url *common.URL) base.Invoker {
	...
	if err != nil {
		logger.Errorf("[Registry][Protocol] consumer service %v create registry directory error, err=%s, and will return nil invoker",
	...
	if err != nil {
		logger.Errorf("[Registry][Protocol] consumer service %v register registry %v error, err=%s",
	...
	if err != nil {
		logger.Errorf("[Registry][Protocol] consumer service %v get cluster %s error, err=%s, will return nil invoker",
			serviceUrl.String(), clusterKey, err.Error())
		return nil
	}
	if cluster == nil {
		logger.Errorf("[Registry][Protocol] consumer service %v cluster is nil for key %s, will return nil invoker",
...
func (proto *registryProtocol) Export(originInvoker base.Invoker) base.Exporter {
	...
	if len(registryUrl.Protocol) > 0 {
		...
		if err != nil {
			logger.Errorf("[Registry][Protocol] provider service %v register registry %v error, err=%s",
		...
		go func() {
			if err := reg.Subscribe(overriderUrl, overrideSubscribeListener); err != nil {
				logger.Warnf("[Registry][Protocol] reg.subscribe(overriderUrl=%v) = err=%v", overriderUrl, err)
			}
		}()
		...
		logger.Warnf("[Registry][Protocol] provider service %v do not regist to registry %v, possible direct connection provider",
...
func (proto *registryProtocol) reExport(invoker base.Invoker, newUrl *common.URL) {
	...
	if oldExporter, loaded := proto.bounds.Load(key); loaded {
		...
		if err := registerServiceMap(invoker); err != nil {
			logger.Errorf("[Registry][Protocol] reExport failed, err=%s", err.Error())
		}
...
func (proto *registryProtocol) Destroy() {
	proto.bounds.Range(func(key, value any) bool {
		...
		if err := reg.UnRegister(exporter.registerUrl); err != nil {
			logger.Warnf("[Registry][Protocol] unRegister consumer url failed, url=%s err=%v", exporter.registerUrl.String(), err)
		}
...
func (proto *registryProtocol) unsubscribeOverrideListener(reg registry.Registry, overrideURL *common.URL) {
	...
	if !ok {
		logger.Warnf("[Registry][Protocol] unexpected override listener type %T for %s", listener, overrideKey)
		return
	}

	if err := reg.UnSubscribe(overrideURL, overrideListener); err != nil {
		logger.Warnf("[Registry][Protocol] unsubscribe override url failed, url=%s err=%v", overrideKey, err)
		return
	}
