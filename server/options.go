...
func GetProviderOptionsFromConfig(c *global.ServiceConfig) []ServiceOption {
	...
	if c.TpsLimitRate != "" {
		if i, err := strconv.Atoi(c.TpsLimitRate); err == nil {
			...
			logger.Warnf("Failed to parse TpsLimitRate %q: %v. TPS limit will not be applied.", c.TpsLimitRate, err)
		}
	}
	if c.TpsLimiter != "" {
	...
	if c.Warmup != "" {
		if d, err := time.ParseDuration(c.Warmup); err == nil {
			...
			logger.Warnf("Failed to parse Warmup duration %q: %v. Warmup period will not be applied.", c.Warmup, err)
		}
	}
