...
// protoc-gen-triple is a plugin for the Protobuf compiler that generates
// Go code. To use it, build this program and make it available on your PATH as
// protoc-gen-triple.
//
// The 'triple' suffix becomes part of the arguments for the Protobuf
// compiler. To generate the base Go types and Connect code using protoc:
//
//	protoc --go_out=gen --triple_out=gen path/to/file.proto
//
...
const (
	...
	triplePackage        = protogen.GoImportPath("dubbo.apache.org/dubbo-go/v3/protocol/triple/triple_protocol")
	consumerPackage      = protogen.GoImportPath("dubbo.apache.org/dubbo-go/v3/consumer")
	...
	generatedFilenameExtension = ".triple.go"
	generatedPackageSuffix     = "triple"
...
func main() {
	if len(os.Args) == 2 && os.Args[1] == "--version" {
		fmt.Fprintln(os.Stdout, tri.Version)
		os.Exit(0)
	}
...
func needsWithIdempotency(file *protogen.File) bool {
	for _, service := range file.Services {
		for _, method := range service.Methods {
			if methodIdempotency(method) != tri.IdempotencyUnknown {
				return true
			}
		}
	}
...
func generatePreamble(g *protogen.GeneratedFile, file *protogen.File) {
	...
	if needsWithIdempotency(file) {
		g.P("const _ = ", triplePackage.Ident("IsAtLeastVersion1_6_0"))
	} else {
		g.P("const _ = ", triplePackage.Ident("IsAtLeastVersion0_1_0"))
	}
	g.P()
}
...
func generateClientInterface(g *protogen.GeneratedFile, service *protogen.Service, names names) {
	...
	for _, method := range service.Methods {
		...
		g.P(clientSignature(g, method, true /* named */))
	}
	g.P("}")
	g.P()
}
...
func generateClientImplementation(g *protogen.GeneratedFile, service *protogen.Service, names names) {
	...
	for i, method := range service.Methods {
		...
		methodsStr += "\"" + method.GoName + "\""
	}
	methodsStr += "}"
	g.P("func ", names.ClientConstructor, " (con *", consumerPackage.Ident("Consumer"), ") (", names.Client, ", error) {")
	g.P("if err := con.Assemble(\"", service.Desc.FullName(), "\", ", methodsStr, ", nil); err != nil {")
	g.P("return nil, err")
	g.P("}")
	...
	g.P("con *", g.QualifiedGoIdent(consumerPackage.Ident("Consumer")))
	//for _, method := range service.Methods {
	//	g.P(unexport(method.GoName), " *", triplePackage.Ident("Client"),
...
func generateClientMethod(g *protogen.GeneratedFile, service *protogen.Service, method *protogen.Method, names names) {
	...
	case isStreamingClient && !isStreamingServer:
		g.P("stream, err := c.con.ConsumeClientStream(ctx, \"", service.Desc.FullName(), "\", \"", method.GoName, "\", opts...)")
		g.P("if err != nil {")
		g.P("return nil, err")
		g.P("}")
		g.P("rawStream := stream.(*", g.QualifiedGoIdent(triplePackage.Ident("ClientStreamForClient")), ")")
		g.P("return rawStream, nil")
	case !isStreamingClient && isStreamingServer:
		g.P("triReq := ", triplePackage.Ident("NewRequest(req)"))
		g.P("stream, err := c.con.ConsumeServerStream(ctx, triReq, \"", service.Desc.FullName(), "\", \"", method.GoName, "\", opts...)")
		g.P("if err != nil {")
		g.P("return nil, err")
		g.P("}")
		g.P("rawStream := stream.(*", g.QualifiedGoIdent(triplePackage.Ident("ServerStreamForClient")), ")")
		g.P("return rawStream, nil")
	case isStreamingClient && isStreamingServer:
		g.P("stream, err := c.con.ConsumeBidiStream(ctx, \"", service.Desc.FullName(), "\", \"", method.GoName, "\", opts...)")
		g.P("if err != nil {")
		g.P("return nil, err")
		g.P("}")
		g.P("rawStream := stream.(*", g.QualifiedGoIdent(triplePackage.Ident("BidiStreamForClient")), ")")
		g.P("return rawStream, nil")
	default:
		g.P("triReq := ", triplePackage.Ident("NewRequest(req)"))
		g.P("resp := new(", g.QualifiedGoIdent(method.Output.GoIdent), ")")
		g.P("triResp := ", triplePackage.Ident("NewResponse(resp)"))
		g.P("if err := c.con.ConsumeUnary(ctx, triReq, triResp, \"", service.Desc.FullName(), "\", \"", method.GoName, "\", opts...); err != nil {")
		g.P("return nil, err")
		g.P("}")
		g.P("return resp, nil")
	}
...
func clientSignature(g *protogen.GeneratedFile, method *protogen.Method, named bool) string {
	...
	if method.Desc.IsStreamingClient() && method.Desc.IsStreamingServer() {
		...
		return method.GoName + "(" + ctxName + " " + g.QualifiedGoIdent(contextPackage.Ident("Context")) + ", " +
			"opts ..." + g.QualifiedGoIdent(consumerPackage.Ident("ConsumeOption")) + ") " +
			"(*" + g.QualifiedGoIdent(triplePackage.Ident("BidiStreamForClient")) +
	...
	if method.Desc.IsStreamingClient() {
		...
		return method.GoName + "(" + ctxName + " " + g.QualifiedGoIdent(contextPackage.Ident("Context")) + ", " +
			"opts ..." + g.QualifiedGoIdent(consumerPackage.Ident("ConsumeOption")) + ") " +
			"(*" + g.QualifiedGoIdent(triplePackage.Ident("ClientStreamForClient")) +
			//"[" + g.QualifiedGoIdent(method.Input.GoIdent) + ", " + g.QualifiedGoIdent(method.Output.GoIdent) + "]" +
			", error)"
	}
	if method.Desc.IsStreamingServer() {
		return method.GoName + "(" + ctxName + " " + g.QualifiedGoIdent(contextPackage.Ident("Context")) +
			...
			"opts ..." + g.QualifiedGoIdent(consumerPackage.Ident("ConsumeOption")) + ") " +
			"(*" + g.QualifiedGoIdent(triplePackage.Ident("ServerStreamForClient")) +
...
func generateDubboClientApi(g *protogen.GeneratedFile, service *protogen.Service, names names) {
	...
	handlerOption := triplePackage.Ident("ClientOption")
	wrapComments(g, "GetDubboStub is used for Dubbo to initialize client interface")
	g.P("func (c *", dubboImpl, ") GetDubboStub(httpClient ", triplePackage.Ident("HTTPClient"),
...
func dubboClientField(g *protogen.GeneratedFile, method *protogen.Method, named bool) string {
	...
	if method.Desc.IsStreamingServer() {
		return method.GoName + " func(" + ctxName + " " + g.QualifiedGoIdent(contextPackage.Ident("Context")) +
			", " + reqName + " *" + g.QualifiedGoIdent(triplePackage.Ident("Request")) + "[" +
...
func generateServerConstructor(g *protogen.GeneratedFile, service *protogen.Service, names names) {
	...
	handlerOption := triplePackage.Ident("HandlerOption")
	...
	for _, method := range service.Methods {
		...
		switch {
		case isStreamingClient && !isStreamingServer:
			g.P(`mux.Handle(`, procedureConstName(method), `, `, triplePackage.Ident("NewClientStreamHandler"), "(")
		case !isStreamingClient && isStreamingServer:
			g.P(`mux.Handle(`, procedureConstName(method), `, `, triplePackage.Ident("NewServerStreamHandler"), "(")
		case isStreamingClient && isStreamingServer:
			g.P(`mux.Handle(`, procedureConstName(method), `, `, triplePackage.Ident("NewBidiStreamHandler"), "(")
		default:
			g.P(`mux.Handle(`, procedureConstName(method), `, `, triplePackage.Ident("NewUnaryHandler"), "(")
		}
		g.P(procedureConstName(method), `,`)
		g.P("svc.", method.GoName, ",")
		switch idempotency {
		case tri.IdempotencyNoSideEffects:
			g.P(triplePackage.Ident("WithIdempotency"), "(", triplePackage.Ident("IdempotencyNoSideEffects"), "),")
			g.P(triplePackage.Ident("WithHandlerOptions"), "(opts...),")
		case tri.IdempotencyIdempotent:
			g.P(triplePackage.Ident("WithIdempotency"), "(", triplePackage.Ident("IdempotencyIdempotent"), "),")
			g.P(triplePackage.Ident("WithHandlerOptions"), "(opts...),")
		case tri.IdempotencyUnknown:
			g.P("opts...,")
		}
		g.P("))")
	}
...
func generateDubboProviderBase(g *protogen.GeneratedFile, service *protogen.Service, names names) {
	...
	g.P("func (s *", base, ") BuildHandler(impl interface{}, opts ...", triplePackage.Ident("HandlerOption"), ") (string, ", httpPackage.Ident("Handler"), ") {")
	...
	for _, method := range service.Methods {
		...
		case isStreamingClient && !isStreamingServer:
			g.P(`mux.Handle(`, procedureConstName(method), `, `, triplePackage.Ident("NewClientStreamHandler"), "(")
			g.P(procedureConstName(method), `,`)
			g.P("func(ctx ", contextPackage.Ident("Context"), ", stream *", triplePackage.Ident("ClientStream"),
				"[", g.QualifiedGoIdent(method.Input.GoIdent), "]) (*", triplePackage.Ident("Response"),
		...
		case !isStreamingClient && isStreamingServer:
			g.P(`mux.Handle(`, procedureConstName(method), `, `, triplePackage.Ident("NewServerStreamHandler"), "(")
			g.P(procedureConstName(method), `,`)
			g.P("func(ctx ", contextPackage.Ident("Context"), ", req *", triplePackage.Ident("Request"),
				"[", g.QualifiedGoIdent(method.Input.GoIdent), "], stream *", triplePackage.Ident("ServerStream"),
				"[", g.QualifiedGoIdent(method.Output.GoIdent), "]) error {")
			invocationArgs = append(invocationArgs, "req", "stream")
		case isStreamingClient && isStreamingServer:
			g.P(`mux.Handle(`, procedureConstName(method), `, `, triplePackage.Ident("NewBidiStreamHandler"), "(")
			g.P(procedureConstName(method), `,`)
			g.P("func(ctx ", contextPackage.Ident("Context"), ", stream *", triplePackage.Ident("BidiStream"),
		...
		if twoReturnValsFlag {
			g.P("return res.Result().(*", triplePackage.Ident("Response"), "[", g.QualifiedGoIdent(method.Output.GoIdent),
				"]), res.Error()")
		} else {
...
func generateUnimplementedServerImplementation(g *protogen.GeneratedFile, service *protogen.Service, names names) {
	...
	for _, method := range service.Methods {
		...
		if method.Desc.IsStreamingServer() {
			g.P("return ", triplePackage.Ident("NewError"), "(",
				triplePackage.Ident("CodeUnimplemented"), ", ", errorsPackage.Ident("New"),
				`("`, method.Desc.FullName(), ` is not implemented"))`)
		} else {
			g.P("return nil, ", triplePackage.Ident("NewError"), "(",
				triplePackage.Ident("CodeUnimplemented"), ", ", errorsPackage.Ident("New"),
				`("`, method.Desc.FullName(), ` is not implemented"))`)
		}
		g.P("}")
...
func clientSignatureParams(g *protogen.GeneratedFile, method *protogen.Method, named bool) string {
	...
	if method.Desc.IsStreamingClient() && method.Desc.IsStreamingServer() {
		...
		return "(" + ctxName + g.QualifiedGoIdent(contextPackage.Ident("Context")) + ", " +
			streamName + "*" + g.QualifiedGoIdent(triplePackage.Ident("BidiStream")) +
			") error"
	}
	if method.Desc.IsStreamingClient() {
		// client streaming
		return "(" + ctxName + g.QualifiedGoIdent(contextPackage.Ident("Context")) + ", " +
			streamName + "*" + g.QualifiedGoIdent(triplePackage.Ident("ClientStream")) +
			") (*" + g.QualifiedGoIdent(triplePackage.Ident("Response")) + " ,error)"
	}
	if method.Desc.IsStreamingServer() {
		// server streaming
		return "(" + ctxName + g.QualifiedGoIdent(contextPackage.Ident("Context")) +
			", " + reqName + "*" + g.QualifiedGoIdent(triplePackage.Ident("Request")) +
			", " + streamName + "*" + g.QualifiedGoIdent(triplePackage.Ident("ServerStream")) +
			") error"
	}
	// unary
	return "(" + ctxName + g.QualifiedGoIdent(contextPackage.Ident("Context")) +
...
func methodIdempotency(method *protogen.Method) tri.IdempotencyLevel {
	methodOptions, ok := method.Desc.Options().(*descriptorpb.MethodOptions)
	if !ok {
		return tri.IdempotencyUnknown
	}
	switch methodOptions.GetIdempotencyLevel() {
	case descriptorpb.MethodOptions_NO_SIDE_EFFECTS:
		return tri.IdempotencyNoSideEffects
	case descriptorpb.MethodOptions_IDEMPOTENT:
		return tri.IdempotencyIdempotent
