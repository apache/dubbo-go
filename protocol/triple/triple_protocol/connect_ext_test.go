...
//	//		response := triple.NewResponse(msg)
//	//		err1 := stream.CloseAndReceive(response)
//	//		assert.Nil(t, err1)
...
//	//		err1 := stream.CloseAndReceive(got)
//	//		assert.Nil(t, err1)
...
//	//		err1 := stream.Receive(response)
//	//		assert.True(t, errors.Is(err1, io.EOF))
//	//		assert.False(t, triple.IsWireError(err1))
...
func TestGRPCMissingTrailersError(t *testing.T) {
	...
	//	assert.Nil(t, err)
	//	assert.Nil(t, stream.CloseRequest())
	//	err1 := stream.Receive(&pingv1.CumSumResponse{})
	//	assertErrorNoTrailers(t, err1)
	//	assert.Nil(t, stream.CloseResponse())
	//})
}
...
//	err1 := stream.CloseAndReceive(res)
//	assert.Nil(t, err1)
//	assert.NotNil(t, res)
...
func TestConnectHTTPErrorCodes(t *testing.T) {
	...
	checkHTTPStatus := func(t *testing.T, connectCode triple.Code, wantHttpStatus int) {
		...
		err1 := connectClient.Ping(context.Background(), triple.NewRequest(&pingv1.PingRequest{}), triple.NewResponse(&pingv1.PingResponse{}))
		assert.NotNil(t, err1)
	}
