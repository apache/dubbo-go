...
func (m *tripleUnaryMarshaler) Marshal(message any) *Error {
	...
	if err != nil {
		if m.backupCodec != nil && m.codec.Name() != m.backupCodec.Name() {
			logger.Warnf("[Triple][Protocol] failed to marshal message with primary codec %s, trying fallback codec %s", m.codec.Name(), m.backupCodec.Name())
...
func (u *tripleUnaryUnmarshaler) Unmarshal(message any) *Error {
	...
	if err != nil {
		if u.backupCodec != nil && u.codec.Name() != u.backupCodec.Name() {
			logger.Warnf("[Triple][Protocol] failed to unmarshal message with primary codec %s, trying fallback codec %s", u.codec.Name(), u.backupCodec.Name())
