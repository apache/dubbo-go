...
func NewClient(httpClient HTTPClient, url string, options ...ClientOption) *Client {
	...
	client.callUnary = func(ctx context.Context, request *Request, response *Response, method string) error {
...
func (c *Client) CallUnary(ctx context.Context, request *Request, response *Response, method string) error {
	if c.err != nil {
		return c.err
	}
	...
	return c.callUnary(ctx, request, response, method)
}
...
// normalizeClientProcedure mirrors protocol/triple/triple_protocol/protobuf_util.go's extractProtoPath but strips empty segments
// to keep the service/method intact for client-side procedure building.
func normalizeClientProcedure(path string) string {
	...
	if len(cleanSegments) > 1 {
		pkg = cleanSegments[len(cleanSegments)-2]
		method = cleanSegments[len(cleanSegments)-1]
	}
	if pkg == "" {
		return "/"
	}
