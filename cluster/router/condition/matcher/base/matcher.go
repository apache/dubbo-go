...
func (c *ConditionMatcher) GetSampleValueFromUrl(conditionKey string, sample map[string]string, param *common.URL, invocation protocol.Invocation) string {
...
func (c *ConditionMatcher) IsMatch(sample map[string]string, param *common.URL, invocation protocol.Invocation, isWhenCondition bool) bool {
	value := c.Matcher.GetValue(sample, param, invocation)
	if value == "" {
		// if key does not present in whichever of url, invocation or attachment based on the matcher type, then return false.
		return false
	}

	if len(c.Matches) != 0 && len(c.Mismatches) == 0 {
		for match, _ := range c.Matches {
			if c.DoPatternMatch(match, value, param, invocation, isWhenCondition) {
				return true
			}
		}
		return false
	...
	if len(c.Mismatches) != 0 && len(c.Matches) == 0 {
		for mismatch, _ := range c.Mismatches {
			if c.DoPatternMatch(mismatch, value, param, invocation, isWhenCondition) {
				return false
			}
		}
		return true
...
func (c *ConditionMatcher) GetMatches() map[string]struct{} {
	return c.Matches
}

func (c *ConditionMatcher) GetMismatches() map[string]struct{} {
	return c.Mismatches
}

func (c *ConditionMatcher) DoPatternMatch(pattern string, value string, url *common.URL, invocation protocol.Invocation, isWhenCondition bool) bool {
	for _, valueMatcher := range c.ValueMatchers {
