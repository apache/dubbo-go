...
func (c *ConditionDynamicRouter) Process(event *config_center.ConfigChangeEvent) {
	if event.ConfigType == remoting.EventTypeDel {
		...
		conditions, err := c.GenerateConditions()
		if err != nil {
			logger.Warnf("[condition router]Build a new condition route config error, %+v "+
				"and we will use the original condition rule configuration.", err)
			return
		}
		c.conditionRouters = conditions
	}
}

func (c *ConditionDynamicRouter) GenerateConditions() ([]*ConditionStateRouter, error) {
	if c.routerConfig == nil {
		return make([]*ConditionStateRouter, 0), nil
	}
