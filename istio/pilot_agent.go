...
type OnRdsChangeListener func(serviceName string, xdsVirtualHost resources.EnvoyVirtualHost) error
type OnEdsChangeListener func(clusterName string, xdsCluster resources.EnvoyCluster, xdsClusterEndpoint resources.EnvoyClusterEndpoint) error
	...
	for {
		...
		case event, ok := <-p.updateChan:
			...
			case resources.XdsEventUpdateCDS:
				if xdsClusters, ok := event.Object.([]resources.EnvoyCluster); ok {
			...
			case resources.XdsEventUpdateEDS:
				if xdsClusterEndpoints, ok := event.Object.([]resources.EnvoyClusterEndpoint); ok {
			...
			case resources.XdsEventUpdateLDS:
				if xdsListeners, ok := event.Object.([]resources.EnvoyListener); ok {
			...
			case resources.XdsEventUpdateRDS:
				if xdsRouteConfigurations, ok := event.Object.([]resources.EnvoyRouteConfig); ok {
...
func (p *PilotAgent) callEdsChange(clusterName string) {
	...
	if listeners, ok := p.OnCdsChangeListeners[clusterName]; ok {
		for listenerName, listener := range listeners {
			if ok1 && ok2 {
				logger.Infof("pilot agent callEdsChange clusterName %s listener %s with cluster = %s and  eds = %s", clusterName, listenerName, utils.ConvertJsonString(xdsCluster.(resources.EnvoyCluster)), utils.ConvertJsonString(xdsClusterEndpoint.(resources.EnvoyClusterEndpoint)))
				listener(clusterName, xdsCluster.(resources.EnvoyCluster), xdsClusterEndpoint.(resources.EnvoyClusterEndpoint))
			}
		}
	}
}

func (p *PilotAgent) callRdsChange(serviceName string, xdsVirtualHost resources.EnvoyVirtualHost) {
	p.listenerMutex.RLock()
	defer p.listenerMutex.RUnlock()
...
func (p *PilotAgent) SubscribeRds(serviceName, listenerName string, listener OnRdsChangeListener) {
	...
	if xdsVirtualHost, ok := p.envoyVirtualHostMap.Load(serviceName); ok {
		logger.Infof("pilot agent callRdsChange serviceName, listener %s with rds = %s", serviceName, listenerName, utils.ConvertJsonString(xdsVirtualHost.(resources.EnvoyVirtualHost)))
		listener(serviceName, xdsVirtualHost.(resources.EnvoyVirtualHost))
	}
}
