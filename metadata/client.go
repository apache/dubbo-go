...
func (m *triMetadataServiceV2) getMetadataInfo(ctx context.Context, revision string) (*info.MetadataInfo, error) {
	...
	if res.Error() != nil {
		logger.Errorf("[MetadataRPC] could not get the metadata info from remote provider, err=%v", res.Error())
...
func (m *remoteMetadataServiceV1) getMetadataInfo(ctx context.Context, revision string) (*info.MetadataInfo, error) {
	...
	res := m.invoker.Invoke(context.Background(), inv)
	if res.Error() != nil {
		logger.Errorf("[MetadataRPC] RPC call failed to %s, err=%v", m.invoker.GetURL().Location, res.Error())
		return nil, res.Error()
	}
	...
	if result, ok := rawResult.(*info.MetadataInfo); ok {
		...
		if err := json.Unmarshal([]byte(strValue), metadataInfo); err != nil {
			logger.Errorf("[MetadataRPC] failed to parse JSON string from provider %s, err=%v", m.invoker.GetURL().Location, err)
...
func getMetadataServiceUrlParams(ins registry.ServiceInstance) map[string]string {
	...
	if str, ok := ps[constant.MetadataServiceURLParamsPropertyName]; ok && len(str) > 0 {
		...
		if err != nil {
			logger.Errorf("[MetadataRPC] could not parse the metadata service url parameters to map, err=%v", err)
		}
	}
