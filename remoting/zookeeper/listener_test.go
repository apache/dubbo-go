...
var(
	propertiesPath = "/dubbo/dubbo.properties"
)
...
`
	...
	err = client.Create(propertiesPath)
	assert.NoError(t, err)

	_, err = client.Conn.Set(propertiesPath, []byte(data), 0)
	assert.NoError(t, err)
...
`
	...
	_, err := client.Conn.Set(propertiesPath, []byte(changedData), 1)
	assert.NoError(t, err)
	wait.Wait()
