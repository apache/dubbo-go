...
性能测试结果表明，gRPC 在所有包体大小场景下均显著优于 Dubbo-Go。特别是在小包体（128B）场景下，gRPC 的 QPS 约为 Dubbo-Go 的 5 倍（118,044 vs 19,231），且延迟更低（P99 1.61ms vs 6.37ms）。在大包体（1MiB）场景下，gRPC 仍保持约 4 倍的 QPS 优势（1,453 vs 310），且延迟显著更低（P99 109ms vs 795ms）。不过，Dubbo-Go 在中小包体场景（128B-16KB）下展现出有竞争力的资源效率，在 50 并发下 CPU 占用相当且内存消耗更低。两个框架在所有测试场景下均保持高成功率（>99.9%）。
