...
type clusterImplBalancer struct {
	...
	requestCounter        *client.ClusterRequestsCounter
...
func (b *clusterImplBalancer) UpdateClientConnState(s balancer.ClientConnState) error {
	...
	if b.xdsClient == nil {
		c := client.FromResolverState(s.ResolverState)
		if c == nil {
...
type scWrapper struct {
	...
	// the picker.
	locality atomic.Value // type resource.LocalityID
}

func (scw *scWrapper) updateLocalityID(lID resource.LocalityID) {
	scw.locality.Store(lID)
}

func (scw *scWrapper) localityID() resource.LocalityID {
	lID, _ := scw.locality.Load().(resource.LocalityID)
	return lID
}
...
func (b *clusterImplBalancer) handleDropAndRequestCount(newConfig *LBConfig) *dropConfigs {
	...
	if b.requestCounterCluster != newConfig.Cluster || b.requestCounterService != newConfig.EDSServiceName {
		...
		b.requestCounter = client.GetClusterRequestsCounter(newConfig.Cluster, newConfig.EDSServiceName)
		updatePicker = true
	}
