...
type sampleRow struct {
	...
	ServerWorkMS      int64
	LimiterFound      bool
	LimiterLimitation uint64
	LimiterRemaining  uint64
	LimiterInflight   uint64
}
...
func reportLoop(ctx context.Context, cfg clientConfig, stats *clientStats, snapshots <-chan serverSnapshot) error {
	...
	if cfg.out != "" {
		...
		if err := writer.Write([]string{
			"time_unix", "elapsed_sec", "phase", "started", "success", "rejected", "failed", "success_qps",
			"reject_rate", "latency_avg_ms", "latency_p95_ms", "server_active", "server_max_active",
			"server_work_ms", "limiter_found", "limiter_limitation", "limiter_remaining", "limiter_inflight",
		}); err != nil {
			return err
		}
	}
...
func buildSampleRow(now time.Time, start time.Time, lastReport time.Time, lastSuccess int64, stats *clientStats, snapshot serverSnapshot) sampleRow {
	...
	return sampleRow{
		...
		ServerWorkMS:      snapshot.WorkMS,
		LimiterFound:      snapshot.LimiterFound,
		LimiterLimitation: snapshot.LimiterLimitation,
		LimiterRemaining:  snapshot.LimiterRemaining,
		LimiterInflight:   snapshot.LimiterInflight,
	}
}

func printRow(row sampleRow) {
	fmt.Printf(
		"elapsed=%.0fs phase=%s started=%d success=%d rejected=%d failed=%d qps=%.1f reject_rate=%.1f%% avg=%.0fms p95=%.0fms server_active=%d server_max_active=%d work_ms=%d limiter_found=%t limiter_limitation=%d limiter_remaining=%d limiter_inflight=%d\n",
		row.ElapsedSec,
		row.Phase,
		row.Started,
		row.Success,
		...
		row.ServerWorkMS,
		row.LimiterFound,
		row.LimiterLimitation,
		row.LimiterRemaining,
		row.LimiterInflight,
	)
}
...
func writeRow(writer *csv.Writer, row sampleRow) error {
	return writer.Write([]string{
		...
		strconv.FormatInt(row.ServerWorkMS, 10),
		strconv.FormatBool(row.LimiterFound),
		strconv.FormatUint(row.LimiterLimitation, 10),
		strconv.FormatUint(row.LimiterRemaining, 10),
		strconv.FormatUint(row.LimiterInflight, 10),
	})
}

func parseConfig() clientConfig {
	var cfg clientConfig
	flag.StringVar(&cfg.url, "url", "127.0.0.1:20002", "Triple provider URL")
	flag.StringVar(&cfg.statsURL, "stats-url", "http://127.0.0.1:21002/stats", "server stats URL")
	...
	flag.StringVar(&cfg.out, "out", "rtt_shrink_result.csv", "CSV output path, empty disables CSV")
